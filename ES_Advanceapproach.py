...
def generating_queryterms(query_dict: Dict, es: Elasticsearch):
    
    ## analyze every query, and expand the dictionary
    ## query dictionary consists of query id,category,question and type
    ##
    ##It returns A dictionary with query ids as keys, questions as values, categories as values, and types as keys.
...
def extracting_features(query_terms: List, 
                    doc_id: str, 
                    es: Elasticsearch, 
                    index=INDEX_NAME):
    ##identifies and extracts a query and document pair's query, document, and query-document features.
    ##it will returns a fixed-order list of extracted feature values
...
##learning to ranking feature vectors
def ltr_featurevectors(es: Elasticsearch, training_queries: Dict,k=100, index=INDEX_NAME):
    ...
    for query_id, query_features in training_queries.items():
        ...
        for hit in hits:
            X.append(extracting_features(query, hit['_id'], es, index))
            relevancy = 0 
...
def ltr_prediction(es: Elasticsearch, test_queries: Dict, model, k=50, index=INDEX_NAME):
    ...
    for query_id, query_features in test_queries.items():
        ...
        for hit in hits:
            featur_vectors.append(extracting_features(query, hit['_id'], es, index))
...
training_queries = generating_queryterms(training_queries, es)
test_queries = generating_queryterms(test_queries, es)
